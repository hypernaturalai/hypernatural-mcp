...
1. **Start from a shot list — whose it is decides the path.** Each `Shot 1: …` entry becomes exactly one shot.
   - **User supplied the shot list** (or per-shot detail): preserve their wording, weave in required `@Name`s (a binding edit, not a rewrite), and create immediately — no expansion into Visual/Animation form, no approval detour. The planner does the expanding.
   - **You are drafting the shots** (the ask was prose, a brief, or a vague idea): give every shot a concrete Visual (subjects, exact `@Name`s) and Animation (motion, camera; quoted on-screen speech first) — read [references/shot-writing.md](references/shot-writing.md) first — then **show the draft and get the user's approval before any `create_*` call**. An explicit waiver ("just make it, I trust you") counts as approval: show the list, don't block on a reply.
